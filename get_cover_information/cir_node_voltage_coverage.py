...
def cir_calculate_node_voltage_coverage_main(cir_file):
    """
    根据网表文件计算节点电压覆盖率，检查所有节点是否连通。
    """
    # 获取网表中的所有连接和节点
    ...
    if len(visited) == len(nodes):
        node_voltage_coverage = 100
    else:
        node_voltage_coverage = (len(visited) / len(nodes)) * 100

    return node_voltage_coverage
...
# # 计算节点电压覆盖率
# coverage = cir_calculate_node_voltage_coverage_main(cir_file)

# # 打印结果
# print(f"Node Voltage Coverage: {coverage}%")
